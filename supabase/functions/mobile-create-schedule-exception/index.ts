...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { global_id, doctor_id, clinic_id, exception_date, exception_type, start_time, end_time, reason } = await req.json();

    if (!global_id || !doctor_id || !exception_date || !exception_type) {
      return new Response(
    ...
    console.error("Error in mobile-create-schedule-exception:", error);
    return new Response(
      JSON.stringify({
