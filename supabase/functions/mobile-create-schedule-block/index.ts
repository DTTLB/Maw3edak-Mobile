...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { global_id, doctor_id, clinic_id, block_date, start_time, end_time, block_type, reason } = await req.json();

    if (!global_id || !doctor_id || !block_date || !start_time || !end_time || !block_type) {
      return new Response(
    ...
    console.error("Error in mobile-create-schedule-block:", error);
    return new Response(
      JSON.stringify({
